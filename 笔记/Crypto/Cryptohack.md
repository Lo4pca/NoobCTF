...
过了这么久再回来看Isogenies，跟看天书一样

直接用 https://github.com/GiacomoPope/Castryck-Decru-SageMath 就好，使用方式在`baby_SIDH.sage`里。计算`two_i`的方式见 https://ctftime.org/writeup/36033

### Special Isogenies

CSIDH与 $F_p$ 上的超奇异曲线打交道，从根本上改变了超奇异同源图（supersingular isogeny graph）。这样做的一个后果是 $E(F_p)=Z/(p+1)$ ；有一个唯一的l阶子群生成l-同源

> 如果你见过CSIDH同源图，你可能记得每个顶点都有两条边。虽然这两条边都是 $F_p$ 有理同源（它们的陪域是曲线 $E/F_p$ ），但并不都由 $F_p$ 有理点生成。更多内容在后面的`Twisted CSIDH Isogenies`中。目前，先假设我们计算的所有同源都由 $E/F_p$ 上的l阶有理点生成

### Prime Power Isogenies

对于SIDH，我们使用形如 $2^{e_A}3^{e_B}-1$ 的质数；用 $l^e$ 阶的点做生成器来计算 $l^e$ 阶同源

CSIDH质数是许多指数为1的小质数的积。在CSIDH中计算 $l^e$ 阶同源时，应该找到一个l阶的点，然后计算由这个点生成的同源的陪域；重复以上步骤e次

比较是否回到原本的曲线时记得用`j_invariant`

### Secret Exponents

在CSIDH key交换中，私钥是与整除p+1的奇质数数量等长的整数列表。该数据表示一个指数向量 $[e_0,e_1,e_2,...,e_k]$ ，决定了Alice和Bob的秘密同源路径

### Twisted CSIDH Isogenies

在`Prime Power Isogenies`中，我们探寻了CSIDH曲线上的l-同源图，发现它们是循环的。我们能够计算图中有多少个顶点并逐个遍历，直到回到起始点

我们还知道，给定曲线上的l阶点，似乎只有一种方式遍历图中的点，即图是有向的。这个挑战有关我们如何有效且高效地在图中“倒着走一步”。意味着如果l-同源图有k个点，相比计算k-1个正向l-同源，我们只需要计算一个逆向同源即可后退一步

为此，我们需要计算曲线的二次扭曲 $E^t$ ，将E映射到 $E^t$ 。基于这条扭曲曲线，我们用一个l阶点按常规方法计算一个l次同源。随后，我们对同源的陪域进行“反扭曲”（即计算其二次扭曲），从而完成回溯过程

> 如果计算仅涉及x坐标的同源（x-only isogenies），则无需显式计算其扭曲曲线；只需在有限域 $\mathbb{F}_p$ 中选择随机的x坐标即可。当 $x^3 + Ax^2 + x$ 是平方数时，该点位于曲线E上；否则位于 $E^t$ 上。由此可见，只需检验 $y^2$ 是否为平方数，便足以确定在使用这个仅含x坐标的点计算同源时，应当向前走还是向后走

> 在CSIDH指数向量中，对于正整数，我们从该曲线出发，在l-同源图上前进e步；对于负整数，我们从该曲线的二次扭曲出发，在l-同源图上后退e步

### CSIDH Key Exchange

给定某个指数向量，从 $E_0$ 计算出相应的同源映射后，公钥为该曲线的Montgomery系数A。如果Alice和Bob各自计算出自己的公钥，随后又基于对方的公钥曲线计算同一个同源映射（使用相同的指数向量），那么他们相当于从 $E_0$ 出发，通过一个由两人各自的秘密指数向量之和所确定的同源映射。鉴于加法运算具有结合律，CSIDH（这一事实背后还涉及更深层的原理）同样具备这一特性；因此，他们各自计算出的第二个同源映射的陪域将完全一致。他们的共享密钥便是这条最终曲线的Montgomery系数

实现时不要用division_points找l阶点，非常慢。另外，据`GMO_Goat`所说，用isogeny_codomain_from_kernel直接算陪域要比计算整个同源快

### What's My Kernel

眼力题。注意`P_a, Q_a`与`P_b, Q_b`完全是一样的，所以phi_P和phi_Q也是 $\phi(P_a)$ 和 $\phi(Q_a)$ 。因为同源的核是 $P_a + n * Q_a$ 且同源保持群运算， $\phi(P_a)+n\phi(Q_a)=0$ ，一个离散对数的事

### André Encoding

weil_pairing有个性质：假如 $E\rightarrow E'$ 的同源的度为d，那么 $e_m(\phi(P),\phi(Q))=e_m(P,Q)^d$

也可以不用这个性质，比如`7Rocky`的解法：一番变形后可以得到 $\phi(P)$ 与 $\phi(Q)$ 之间存在线性关系，仅在少数情况，比如得到正确的b时成立

### Abelian SIDH

dual函数的作用是计算某个同源 $\phi$ 的“对偶同源”（dual isogeny），满足 $\hat{\phi}\circ\phi=[deg(\phi)]$ （即 $\hat{\phi}(\phi(G))=deg(\phi)G$ ）。到这就能发现这题和同源一点关系都没有了

不过看`7Rocky`的解法，还需要额外考虑shared secret的符号

### Dual Masters

结合上一题出现的dual函数，发现给出phi的codomain和一个像点的情况下足够算出phi的dual了（因为 $deg(\phi(Q))=deg(\phi)$ ）。但是随后注意到 $dual(\phi(Q))=0$ 。我并不知道这意味着什么，于是就不知道怎么做了，跑去看了前面提到的佬的记录

那位佬的思路和`maple3142`一致，似乎是因为算出phi的dual后再算dual的dual得到的就是phi了。这个性质在`poly.pdf`第15页的`Dual isogeny theorem`的第6条有提到
...
依旧是脚本小子： https://github.com/jvdsn/crypto-attacks/blob/master/attacks/gcm/forbidden_attack.py

`Dmit`提到了一些特殊的性质，使这道题不需要那么复杂的计算：
```
...
链接中对A_subbytes的计算似乎有误，换成`A_subbytes[:, 7 - i]`就能通过assert了。以及新版本的sagemath改动了api名称，fetch_int换成from_integer，integer_representation换成_integer_representation即可

之前见过这类题。AES只有sub_bytes步骤不是线性的。题目使用了完全线性的自制sbox，导致整个密码可以用Ax+b来表示，其中A完全与key无关。虽然b依赖于key和某些常数项，但可以通过一组明文/密文对算出

查看sbox是否线性可以用`sbox[i ^ j] == sbox[i] ^ sbox[j] ^ sbox[0]`条件。如果所有的(i,j)对全部满足这个条件，则sbox是线性的。也可以用sagemath自带的API：`SBox(AES.sbox).polynomials(groebner=True)`，如果输出的多项式全部是一次的，则sbox是线性的

`Jimvy`使用了差分攻击。差分分布表显示，对于每个 ΔX，都存在一个唯一的 ΔY，使得S(X ⊕ ΔX) = S(X) ⊕ ΔY。而其他步骤对差分的影响都是可逆的。因此知道两个密文之间的差分便可以反向推回两个明文之间的差分

### Oracular Spectacular

神秘的数学魔法

没想出来怎么做，不知道有什么好方法区分0.6和0.4。去看了社区服务器中大家对这道题的讨论，预期解竟然有将近100%的准确率。啊？

遇事不决请大佬。他给我描述了这样一个思路：
```
最简单的方法是：

对每个猜测的字符进行相同次数的查询

然后查看结果是更接近正确结果的分布还是错误结果的分布
...
（如果某些字符得分相同，则选择其中任何一个）

如果查询成功，加1分

如果查询失败，减1分

最终，得分最高的那个字符应该就是正确答案
```
以往看完佬给我的提示后，我立刻就能知道为什么某个思路是正确答案；但这回我看完后仍然是懵的。真的假的？为什么这样可以？不管了，先把这个思路“翻译”成代码，然后通过调试判断找到成功字符的条件是什么

于是成功率莫名其妙地提高了，莫名其妙地就得到flag了。啊？

`Neobeo`的实现有99.4%的成功率，神依然稳定发挥

`goreil`用information theory，通过entropy分析了大概需要多少次query，并与上述算法做比较
...
numpy的列表元素类型是int64，意味着存在整数溢出。所以所谓的“和是质数，积也是质数”指的是“积模 $2^{63}$ （有符号整数）是质数”

我以为这题很复杂，没想到直接爆破就好了，速度不慢。有几点需要注意：
- 数字乘积必须足够大
- 只用奇数字符
- 密码长度是奇数（“偶数长度”配合“全奇数字符”的和是偶数）

### Bit by Bit

看数学书的时候见过一道题，说可以用判断是否是quadratic residue的方式泄漏指数的lsb。虽然不知道是否和这题有关系，但试试总没错。于是发现了`pow(c2,(q-1)//2,q)`的值与m的bit值密切相关，然而我并没看出来为什么

看了`Robin_Jadoul`的解答才知道我又又又看错题了，这次是没搞明白符号优先级：`me = padding << (1 + m % 2)`。首先g是QR；再有seed是QR于是padding也是QR。如果m%2为1，则padding\*4也是QR；否则padding\*2不是

### Toshi's Treasure

搜到了这个： https://crypto.stackexchange.com/questions/54578/how-to-forge-a-shamir-secret-share ，然而无法直接套用，因为此处默认真正的S已知

不过解答中给出了秘密构建时的公式。注意到在得知其他的x项后构建的S'等同于“已知项+未知项”，那么利用第一次输入的share和恢复出的S'就能求出未知项，后面的内容就简单了

`josephs`使用矩阵求解

### L-Win

之前见过类似的题， https://www.ruanx.net/babylfsr 的脚本恢复mask（SECRET_T）后跟着 https://github.com/XDSEC/MoeCTF_2022/blob/main/Official_Writeup/Crypto/Moectf_CryptoWriteup.md 逆向回最初的state

看`trillian`的解法发现sagemath自带的berlekamp_massey可以直接求出mask，mask的companion_matrix可以用来模拟lfsr的`_clock`

### Nothing Up My Sleeve

https://blog.bi0s.in/2024/03/28/Crypto/Predictable-bi0sCTF2024
...
和chatgpt与deepseek讨论出了一个思路：把题目用多变量方程 $x_1a_1+...+x_na_n\equiv a_1^{x_1}...a_n^{x_n}\mod 2^{63}$ 建模，然后在模2下找到一个解，随后用爆破慢慢lift到 $2^{63}$ 。确实能解，但是解太大了：长度为18位数字的密码过于抽象了

在讨论过程中chatgpt一度提到格，但是又自我否定了，因指数部分不是线性的。到最后它一直执着于搞2-adic。我决定让它继续想，而我去问问大佬

大佬说“尝试线性化乘积部分”。我没明白，但是突然想到chatgpt说的格，于是回了一句“不会是格吧”？佬看我还不太懂，直接把一切摊开了说：“用两个方程，乘积部分用离散对数线性化”。我意识到我好像见过这个技巧。让每种字符的数量为短向量；原本的和已经是线性的，乘积部分找个生成器g，原本的乘方就变成指数之间的线性运算了。好的我懂了

实际是我不懂。 $Z/2^{63}$ 不是循环群，自然就没有生成器g。chatgpt仍是执着于2-adic，找了个2-adic logarithm；但很可惜这个函数没有 $log(a^x)=xlog(a)$ 的性质

deepseek说可以将群结构分解成 $(-1)^{\epsilon}5^t\mod 2^{63},\epsilon\in 0,1$ 。意味着可以把5看成生成元，只是阶为 $2^{61}$ 。我由此构造了格，但是得出的解还是太大了。两个AI都说我的格构造错了，结果它们修改后的脚本完全不能跑

在几个小时的绝望后，我终于发现了问题：选择的p太大了。 $2^{16}$ bit的p就差不多了

一些solutions区的发现：
- 大部分人的模数都是 $2^{64}$ 。这点没啥影响
...
flag长度为128，大概是1024位。只要恢复大概5个256位的质数，其乘积的位数就能超过1024，然后就能在 $p_i$ 上解密出 $m\mod p_i$ +crt恢复完整的flag了

### 1337crypt

https://jsur.in/posts/2020-09-20-downunderctf-2020-writeups

倒是能看出来是coppersmith，然而和deepseek哼哧哼哧分析了半天，没有一个人想到构造以 $D\sqrt{p}$ 或 $D\sqrt{q}$ 为根的多项式；思路一直卡在把hint乘方，恢复p+q的高位，然后死于不知道怎么构造coppersmith多项式
...
我以为这题和我之前做过的“部分信息泄漏”题目有关系，这类题目通常用dfs剪枝爆破出p和q。q的每个十进制位的可能取值只有0，1和2，那么逐位爆破q的十进制位并同步计算p，然后检查两者的乘积是否等于n的低位

问题出在“同步计算p”。q的十进制位是p的三进制位，没有办法同步从三进制位推出p的十进制位。我问ds如何解决这个问题，它第一次没读懂我的意思，把我的脚本复述了一遍；第二次直接指出可利用单调性二分搜索，给出的脚本完美运行

`q=d(p)=int(gmpy2.digits(p, 3))`是严格单调递增的，所以`n=f(p)=p\*d(p)`也是严格单调递增函数。用二分搜索满足f(p)=n的p的值即可
...
Cⱼ = Pⱼ ⊕ Oⱼ           # 与明文异或得密文
```
如果明文长度只有16字节，两者的输出相同。所以在cfb模式输入16个`\x00`即可获取ofb模式下的异或密钥，得到前16字节的flag

因为cfb使用密文作为下一轮输入，所以在拿到密文后可以用ecb获取剩下的异或密钥

### Tough decisions

依旧看了半天啥也没看出来。只知道由于flag是ascii字符，部分0 bit位置已知，可以确认 $a_is+e_i$ 的结果。线性方程肯定不行，格的话噪声又太大了；爆破噪声更是不可能

我已经不知道在细节上栽了多少次跟头了。[wp](https://7rocky.github.io/en/ctf/other/ecsc-2023/tough-decisions)指出，sample_noise只会返回偶数，用`Nativity`里的技巧解出s模2后比较附件给出的结果与实际的 $as+e\mod 2$ 是否一致即可判断0/1

（这是因为`e <<= 1`是循环的最后一个语句）

### Blind

ds指出题目是一个Schnorr签名变种，并直接给出了求解脚本。原理倒不难理解：每组签名由`(r,z)`组成，根据sign中的表达式，zG=(ω + c * x)G。由于`c = int.from_bytes(H(r), "big")`，我们可以算出c；再利用已知的Y=-xG，可进一步计算ωG=zG+cY。注意计算r时只用到了ωG的x坐标和m1，于是得到ωG后便可算出m1。`m1 = F1(m) + xor(F2(F1(m)), m)`，取前40个字节即为F1(m)，然后用`F2(F1(m))`与m1剩下的字节异或得到m'。最后比较F1(m)是否等于F1(m')就能判断这是否是真实的签名

找到正确的k后，脚本卡在`bcrypt.kdf`是正常的。题目刻意使用大轮数防止爆破

### 1n_jection

ds说这是康托尔配对函数（Cantor pairing function），并且该函数是双射。逆向这个函数需要注意到在`z=((i+j)*(i+j+1))//2+j`中，若将i+j整体看成w，则`w(w+1)/2`是三角数（可排列成等边三角形形状的点数。第 n 个三角数 $T_n$ 就是前 n 个自然数的和）。康托尔配对函数π(x, y) 的核心是，它会根据 x 和 y 的和 w = x + y，把数对放到一个特定的“层”里。每层的编号就是w，每层的起点正好就是w对应的三角数，层内按照 y 的值从小到大排列。举几个例子：

- w=0：和为 0 的数对只有(0,0)。由于不存在任何和小于0的数对，该层的起点为0，对应三角数 $T_0=0$
- w=1：和为 1 的数对有(0,1), (1,0)。第 0 层已存在一个数对，所以这一层的起点编号是1，对应三角数 $ T_1 = 1 $
- w=2：和为 2 的数对有(0,2), (1,1), (2,0)。前面已有三个数对，于是起点编号是3,对应三角数 $ T_2 = 3 $

所以关键是确定z对应的数对在哪一层，即解满足不等式 $\frac{w(w+1)}{2}\leq z$ 最大的w。求解 $w^2+w-2z=0$ 的正根并向下取整就是我们要找的w，之后就显而易见了

### U-turn

我看到矩阵的第一反应是求逆，随后意识到这不是方阵。不过要求解的x向量的各个分量都只有五种可能，完全可以爆破缺失的的两个维度，得到48 x 48的方阵A'。然而我又忘了这是 $Z_{256}$ 下的方阵，只要分量出现偶数就不可逆。不过不可逆并不代表方程组不可解。找出一个满足Ax=b的特解后组合所有齐次解（所有满足Ax'=0的x'）便能得到所有解

计算A'的right_kernel_matrix发现只有一个齐次解。这下就好办了，逐个检查x+kx'看看有没有满足要求的解（而且因为是 $Z_{256}$ ，k只有256种可能），然后爆破sanitize里被截断的部分，逆向btq并异或固定的padding字符即可

### Additional problems

ds的专家模式好像确实比快速模式聪明很多

最开始以为这题是agcd，因为给出的oracle泄漏的信息其实很少，可能还是得从给出的flag密文直接LLL出p？并非如此，参考笔记里`uf`这题，我们的方程并不满足“ $y_1z_0-y_0z_1$ 很小”这点

继续观察脚本，注意到add功能没有限制次数。如果多次累加使得Nr部分大于p会发生什么？实验发现这会导致解密给出错误的值。把这个现象描述给ds后，它立刻给出了解法：
- 加密0可得到 $c_i=pq_i+Nr_i$
- 累加k个0的密文可得到 $C_k=\sum_{i=1}^kc_i=p\sum q_i+N\sum r_i$
- 解密得到 $(N\sum r_i\mod p)\mod N$
