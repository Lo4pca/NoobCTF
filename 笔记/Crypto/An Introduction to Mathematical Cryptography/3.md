...
subexponential的定义在169页（`Exercise 3.30`见过但我忘了）：

$\forall\alpha,\beta$ > 0,无论 $\alpha$ 有多大， $\beta$ 有多小，有 $\Omega(ln(X)^{\alpha})=f(X)=O(X^{\beta})$

仍然是取对数：

$ln(F_{a,b}(X))=ln(X)^{\frac{1}{a}}ln(ln(X))^{\frac{1}{b}}$ , $ln(ln(X)^{\alpha})=\alpha ln(ln(X))$

$ln(ln(X))^{\frac{1}{b}}$ 在b趋近于0时趋近无穷，因此有 $\Omega(ln(X)^{\alpha})=f(X)$

$ln(X^{\beta})=\beta ln(X)$ 。这里可以让 $\beta\geq ln(X)^{\frac{1}{a}}$ ，但我不确定是否无论b多小，ln(X)均大于 $ln(ln(X))^{\frac{b}{1}}$