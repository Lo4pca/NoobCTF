...
- `set *(char **)addr = value`
    ...
    - 以及32位下修改addr处的内存（4个字节）：`set *(unsigned int*)addr=value`。但今天测试后发现直接`set *addr=value`就行。不过这种方式只能设置32bit
    - set命令可以直接设置寄存器的内容为某个字符串。如`set $rdi="/bin/sh"`。但似乎不能直接将某个地址的内容设为字符串
...
- `jump <addr>`
    - 跳转到某个地址
- `p &name_of_symbol`
    - 获取指定symbol的偏移/地址（运行时）
- `finish`
    - Execute until selected stack frame returns
- 可以用gdbserver跨docker container调试。首先在container内运行：
```sh
docker run -it -p 2345:2435  ... bash
apt update && apt install -y gdbserver
gdbserver 0:2345 ./chal 
```
然后本机运行：
```sh
...
- `catch syscall xxx`：捕捉指定syscall调用。一般反调试会调用ptrace，捕捉后即可绕过
- 同时调试多进程： https://justinapplegate.me/2025/ductf-mysteriousvault （`Debugging`）
- 一些pwndbg独有的[命令](https://browserpwndbg.readthedocs.io/en/docs/)
    ...
    - `attachp name`
        - attach到名为`name`的进程。见 https://github.com/pwndbg/pwndbg/issues/2301
...
- `docker run -it -v $(pwd):pwd2 <image_id> /bin/bash`
    - 与docker container共享目录。本机上的当前目录等同于container内部的pwd2目录
    - image id也可以是云端上的ubuntu image，比如`ubuntu@sha256:xxx`
- `docker compose up -d`
    - 运行`docker-compose.yml`。旧版本的命令为`docker-compose up -d`，有时遇见较新的`docker-compose.yml`会报错
- `docker compose down -v --rmi all`
    - 清理已经build的容器。如果之前已经build过容器但后续修改了`docker-compose.yml`，需要清理后才会重新build
...
连接gdb调试： https://hackmd.io/PzxSZd9DQ0uyqaR-idvPAA
...
今天发现两个好像都down了……找了个替代的： https://www.file.io/ 。这个的api稍微难用点，但是模板如下：
```sh
curl -X 'POST' \
  'https://file.io/' \
  -H 'accept: application/json' \
...
## [A Block and a Hard Place](https://ctf.krauq.com/bi0sctf-2024)
...
- 安装32位gcc库：`sudo apt-get install gcc-multilib`