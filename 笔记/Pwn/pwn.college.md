...
malloc取出tcache堆块时会清空key值。因此虽然我们无法分配到secret的起始地址，但我们可以分配到`起始地址-8`，直接清空secret的前8个字符。接下来再分配到`起始地址+8`读取剩下的secret即可

### Echo Emanations(Easy)

这题没有泄漏任何地址，于是我想都没想就准备使用tcache一条龙：tcache+unsorted bin泄漏libc->`__environ`泄漏栈地址->栈上写rop链。结果实施时我竟然得到了一个elf地址？调试发现我忘记在malloc时加padding chunk，导致free的第8个堆块被合并进top chunk，自然没有libc地址；elf地址则是来自echo函数。该函数调用malloc时libc会从top chunk取出一部分——地址正好是我分配的第8个chunk——然后写入`/bin/echo`字符串的地址

修一下脚本，现在我既有libc地址，也有elf地址了。然后我突然感觉不对。题目给了win函数，`Stack`出现在同系列的后面几道题目名称中——是不是还没有讲到`__environ`？不用`__environ`是否可以做出这道题？
...
主程序根据flag的字符c访问`0x1337000+c*0x1000`。因为cpu会缓存访问后的页面，后续我们用子程序逐个访问页面时，访问主程序访问过的页面的速度要快于没有访问过的页面

剩下的问题是如何测量访问页面所需的时间。答案是用内联汇编调用`rdtsc`指令，它可以读取CPU自启动以来的周期数。另外由于cpu可能用乱序执行提高程序性能，需要用`mfence`（保证在完成指令之前的所有内存读写操作后才执行后续指令）和`lfence`（确保指令之后的内存读取操作不会在指令之前执行）包住`rdtsc`，防止`rdtsc`额外测量到其他指令的耗时

### Baby Spectre 4

参考上一题的`flush_cache`，需要在子程序内手动刷新缓存。注意clflush会刷新指定地址所在的整个缓存行（一般是64字节），因此`clflush(addr)`刷新的是`[addr, addr+63]`。不难发现`flush_cache`并没有完整刷新全部的256个页面，只有每个页面起始的64字节；但考虑到主程序访问页面时只读取页面的第一个字节，这点无伤大雅

### Baby Spectre 5

cpu中有一个分支预测器，它会根据某个分支的历史跳转情况预测后续采用的分支。比如我们多次发送`*index=258`的信号使if条件为真，则cpu会更倾向于if条件为真的分支A；后续执行时cpu会暂时忽略if条件，提前执行分支A中的代码。spectre攻击正是利用这点：在训练cpu采用某条路径后，cpu可能会无视条件语句而执行本不应该执行的语句，在缓存中留下痕迹；即使后续cpu用某种方法检查出条件为假并回滚到执行之前的状态，缓存中的痕迹却不会被清空

实操时由于环境中存在大量噪声，仅测量一次是不够的。ds给我制定了以下策略来消除噪声的影响：
- 运行攻击多次
...
虽然程序打开了多个ypu，但能用到的也就一个，因为不同ypu之间的状态不共享。利用的重点仍然是并行的连接之间的race condition：一组线程不断调用run_ypu，另一组线程不断调用init_ypu加载不同的程序。整体思路不难，但是细节上需要注意：
- 交互程序的菜单只会打印一次，不能用sendlineafter控制何时输入。前期做准备工作时（比如用load_program加载两个程序）各个指令之间最好加个sleep
- 每次ioctl都会清空当前ypu的数据，包括已打开的文件。我本来想着把yan85代码分成两部分，一部分加载文件名并打开文件，只有另一部分参与实际竞争。自然因为所有fd都被清除，即使竞争成功了也没法读取flag
- 不能用过多线程。我用了10个，每组5个。似乎太多线程会导致程序出现非预期行为
- 打开文件时不要用O_CREAT标志。竞争状态下构造的文件名不一定是期望的内容，O_CREAT可能创建出多个没用的文件。一旦打开的文件数超过8就没法再打开文件了
- 使带出flag的文件名尽量短，比如/tmp/flag。家目录太长，更有出现文件名被截断的问题
- vm和宿主机的/tmp目录不互通
