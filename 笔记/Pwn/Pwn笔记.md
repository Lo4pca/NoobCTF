...
- [cornelslop](https://kqx.io/writeups/cornelslop)
  - RCU回调上发生的double free（uaf）：代码在check_entry函数中存在条件竞争，该函数会用RCU回调free损坏的entry，但并未检查entry是否已被free
  - victim object来自自定义cache，因此需要实施cross cache攻击。由于同一时刻的分配数量限制在128，需要利用多个cpu：
    - 每个cpu申请对象时会遵循以下顺序：Per-CPU Freelist (对象) -> Per-CPU Partial List (Slab) -> Node Partial List (Slab，即共享 Partial List) -> 伙伴系统 (Buddy System)
    - 通过在core 0申请对象，在core 1释放对象可以做到清空core 0的freelist，填满core 1的freelist。当core 1的freelist指向的slab变为“部分空闲”（只有victim对象仍在使用）后，这个slab会被放入Per-CPU Partial List；在Per-CPU Partial List中的Slab数量超过阈值时，内核会批量迁移整个列表到Node Partial List；最后当victim被free后，victim所在的slab会被释放回Buddy System
    - 当victim所在的slab回到Buddy System后，可以申请这个内存页的函数就不限于SLUB使用者了，后续我们可以用pipe+write抢占这块内存。由此完成cross cache攻击
  - 抢占（Reclaiming）上述内存块也需要多个cpu配合：
    - RCU的free操作会把对象放入队列，并等待一个“grace period”后才会执行。这需要我们在victim对象的两次free中间插入足够多的“占位slab”，尽量延长Reclaiming操作可用的时间
    - 单个cpu无法同时执行RCU回调和抢占内存块的操作，所以需要将任务分配给多个cpu：cpu 3执行victim的第一次free，cpu 2提供占位slab，cpu 0触发条件竞争的double free
  - 文章末尾的exp注意以下几点：
    - ROUNDS1对应cross cache攻击，ROUNDS2的作用是填充cpu 2的freelist，这样后续批量spray slab时会更快
    - pipe创建时并不分配数据缓冲区，真正的内存分配发生在第一次调用 write() 时
  - 其他可重用的技巧：
    - [EntryBleed: Breaking KASLR under KPTI with Prefetch](https://www.willsroot.io/2022/12/entrybleed.html)：一种侧信道泄漏kernel地址的方法
    - 一个可以在控制rdi指向的内容时使用的gadget：该gadget会把rdi指向的内容作为iret frame恢复并切换回用户态。这将允许我们控制eflags中的IOPL标志。如果题目在qemu环境下，可以直接用 https://kqx.io/post/fw_cfg 的方法越权读取文件
  - 另一篇[wp](https://ptr-yudai.hatenablog.com/entry/2026/03/16/174349)延长竞争窗口和rce的思路完全不同
    - 用madvise+MADV_DONTNEED标志清空指定内存区域对应的PTE（页表项），清除虚拟地址与物理内存的映射。这会导致check_entry读取这块内存时得到page fault，需要处理缺页异常
    - 用另一个进程反复调用mprotect修改同一块内存区域的权限。这会导致两个进程不断争抢整个内存区域的写锁（mmap_write_lock），大幅延长竞争窗口
    - 这个技巧在程序会遍历一大块虚拟内存区域时很好用，可以拖慢遍历过程但不完全堵死
    - rce的思路是利用PTE Overlap写文件：利用double free控制一个页表页（PTE Page），使得该页表页同时包含只读文件的PTE和匿名内存区域的PTE
- [throughthewall](https://icctx.xyz/blog/throughthewall)
  - [DirtyPipe](https://0xnull007.github.io/posts/dirtypipe-cve-2022-0847)：越权写入磁盘上的任何文件
    - 这个技巧利用了splice和pipe_buffer的PIPE_BUF_FLAG_CAN_MERGE标志。splice可以在pipe和另一个fd之间传递数据，通过直接传递对page的引用实现zero-copy。可以简略地理解成`pipe->bufs[i]=fd->data`
    - 当用户写入pipe时，kernel需要考虑当前写入的pipe_buffer是否是对某个文件数据的引用。由此催生出`PIPE_BUF_FLAG_CAN_MERGE`标志，用于决定是否可以在当前pipe_buffer末尾追加数据
    - 早期linux版本在写入pipe_buffer对文件的页引用时没有清空标志，导致攻击者可以利用残留的标志写入任何文件
  - 这题存在uaf漏洞，可以直接堆喷pipe_buffer并修改标志
- [🍼🤏🤏](https://kqx.io/writeups/baby_smallest)
  - 题目允许用户修改percpu变量`TOP_OF_STACK`，该变量在上下文交换（context switching）的过程中记录栈指针
  - 用SGDT泄漏sp0地址： https://kqx.io/post/sp0 。仅可在未实现UMIP（如qemu和早期linux）的设备中使用
  - 内核处理中断（interrupt）的过程的简要trace为`asm_exc_*(->error_entry(->sync_regs))->exc_*`。在sync_regs中内核将当前栈上的`pt_regs`结构体拷贝到`TOP_OF_STACK`。然而正常执行函数时函数在拷贝前会检查rsp是否在`[current->stack, current->stack + THREAD_SIZE)`范围内，考虑到在开启kaslr的情况下唯一知道的内核地址是sp0，只能将栈（sp0）上的数据作为目标
    - wp决定修改sync_regs的返回地址。由于地址未知，只能尝试partial overwrite。但因为`pt_regs`结构体的最后一个字段ss的高位字节固定为`\x00`，实际只能覆盖返回地址的lsb为null
    - sync_regs原本的返回地址是某个`asm_exc_*`。在所有`asm_exc_*`函数中，wp选择了`asm_sysvec_apic_timer_interrupt`-`asm_spurious_interrupt`路径，因为这个路径允许攻击者伪造整个`pt_regs`，从而控制ss，用上述技巧实现真正的partial overwrite
      - `asm_spurious_interrupt`会调用`entry_error`，执行切换上下文，将gs换成用户态 TLS 指针，根据gs取percpu变量`TOP_OF_STACK`。由于程序开启了fsgsbase，攻击者可以将gs换成kernel中的地址，再次控制`TOP_OF_STACK`
  - 最后需要找一个跳板用伪造的iret frame返回到用户态，利用eflags和 https://kqx.io/post/fw_cfg 读取flag。wp找到的跳板为`restore_regs_and_return_to_kernel+12`，它会清理`error_entry`压入栈的上下文，交换到用户态的页表，并执行iretq
  - 非预期解利用 https://github.com/bcoles/kasld/blob/master/src/components/perf_event_open.c 泄漏地址
- [krwd](https://kqx.io/writeups/krwd)
  - 内核进程异步的特殊行为：用户进程的`current->mm`指向对应的用户空间内存布局，而内核进程的`current->mm`为NULL。在内核进程中调用`copy_to_user`或`copy_from_user`时，当前内核进程会“借用”一个用户进程的内存描述符。因此异步读写功能可能发生“在A进程读但是写入B进程的内存”的情况
  - 简化版fsop：对于静态链接的文件，`_IO_vtable_check`将跳过对该文件的vtable检查，允许直接利用vtable实现fsop
  - 另一种getshell思路 https://blog.rawpayload.com/blog/trx-ctf-2026-kwrd-writeup ：泄漏busybox的堆地址并覆盖退出命令
...
- [Push pull pops](https://vmpr0be.com/post/securinets_ppp)
  - capstone中存在一个[issue](https://github.com/capstone-engine/capstone/issues/2442)，遇见`movsxd`指令时会报错
- [Registered Stack](https://github.com/srdnlen/srdnlenctf-2026_public/blob/main/pwn_registered_stack)
  - 仅用push和pop指令且操作数只能是寄存器的shellcode
  - 思路是利用rsp。将要修改的值存入寄存器rsp，随后利用push和pop便能修改rsp中的值；最后再用一组push和pop取出rsp中的值。不过这种方法能修改的值有限，只能加减2或8（对应能控制的寄存器大小），且无法获取奇数字节。奇数字节需要在未清空的寄存器里找，比如fs
...
- 更详细的解析和非预期解： **heap01** 。非预期解利用了之前见过的一个点，分配一个大chunk会导致malloc使用mmap分配内存，且这块内存在libc之上，偏移固定；然后利用索引越界覆盖libc的got表getshell。然而我自己调试的时候发现大chunk所在地在libc下面，而且中间有块guard page。这导致这个chunk与libc的偏移不固定。我用的libc和远程是一个版本的，可能是机器的问题
...
- 分析coredump文件
- 类似的题目：[crash](https://tedthecaver.github.io/2024/11/21/HeroCTF.html)
