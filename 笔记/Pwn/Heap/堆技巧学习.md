...
是时候放个新版本的double free了。不太确定是从哪个版本开始（应该是2.30+），没法直接在tcache里搞double free。how2heap里和double free（dup）有关的有下面三条（拿2.39举例）：
- https://github.com/shellphish/how2heap/blob/master/glibc_2.39/fastbin_dup.c
- https://github.com/shellphish/how2heap/blob/master/glibc_2.39/fastbin_dup_consolidate.c
- https://github.com/shellphish/how2heap/blob/master/glibc_2.39/fastbin_dup_into_stack.c

全是fastbin了。一是最基本的double free poc，展示可以用double free两次取到同一个chunk；三是利用，可以通过double free拿到栈上的chunk。注意代码里的`stack_var[1] = 0x20;`是必须的，也就是说这种方法不能完全做到任意地址分配，需要保证目标位置有正确的metadata

二严格意义上讲不算是double free。calloc会跳过tcache取chunk，直接用`_int_malloc`；因此free chunk后chunk会被直接放进fastbin。此时malloc一个大chunk（>= 0x400）会触发malloc_consolidate，把那个fastbin里的chunk合并进top chunk。于是malloc返回的地址和之前calloc返回的地址是一样的。free两者中任意一个都会构造出uaf

这三种做法都依赖calloc。直接用malloc也是能搞出double free的，而且是纯粹的任意地址分配。只需要先填满tcache，剩下的chunk就会进fastbin；接着在fastbin里double free；然后清空tcache。此时fastbin里的chunk会被放到tcache里。tcache里没有metadata的检查，故不需要保证目标位置有正确的metadata。简直是完美的扬长避短（tcache没有metadata检查，但是有double free检查；fastbin没有double free检查，但是有metadata检查）。真要挑缺点的话只能说这种方法需要的malloc次数较多，有些题目会限制调用malloc的次数

例题[pizzeria](https://github.com/MarcoPellero/writeups/tree/main/backdoor/pizzeria)

### 利用double free获取chunk overlapping和tcache poisoning

今天看了[Free the monsters](https://github.com/tesuji/ctf-writeups/tree/master/2025/trx/pwn/free)的wp，题目的漏洞为：
- double free
- 堆地址泄漏

但无法往已经free后的chunk里写东西。没想到单纯double free可以延伸为chunk overlapping+tcache poisoning，从而达到任意地址写。特此记录一下

`run.py`中main函数里的内容均为准备工作，比如泄漏堆地址。`alloc(0, fakechunk)`则是布置后续堆利用中fakechunk的重要信息，比如prev_size，size，fd之类的。若缺少这步，后续就没法free fakechunk了。能够free fakechunk是漏洞可以重复利用的关键

然后看`heap_atk`函数里的`create an overflapping chunk using fastbin`这步。前半部分没啥好说的，标准的"填满tcache->padding chunk防止空闲堆块合并至top_chunk->fastbin里触发double free"。目前fastbin里是这样一个布局（注意其实B的fd里存的也是A）：
```
A -> B <- A
```
`drain tcache`后的步骤如下：
```py
alloc(a, atk=fakechunk_addr ^ (fakechunk_addr >> 12)) #末尾的a被取出，借此机会修改a的fd
alloc(b) #b被取出
alloc(a) #b的fd里是a，所以又取出了a
alloc(victim) #a的fd在最开始被修改了，于是这里就取出了fakechunk（victim）
```
清空tcache后若再malloc一次，A会被取出来，剩下的堆块都会进到tcache里。但是进入tcache的堆块顺序是反着的，所以我们修改A的fd后，tcache里的布局如下：
```
B -> A -> fakechunk
```
成功将fakechunk链入tcache。此时已经实现了chunk overlapping，因为fakechunk位于main函数alloc的第0号堆块里，在程序规定的0x40输入大小下能够向下溢出到第c号堆块的fd

到`prepare 2 fakechunks for fake largechunk`这步。需要利用fakechunk创造出另一个size较大（unsorted bin size）的假堆块（fake_largechunk，根据脚本，位于next_fakechunk_addr），从而free这个假堆块得到libc地址。创造这步倒不难，只需要free与victim相邻的c后再free victim，利用victim修改c的fd（tcache poisoning）便能获取在任意地址处的chunk了。唯一需要注意的是脚本里`alloc(d, fakechunk2, defense=0x21)`这步(注意fakechunk2不是fake_largechunk)。我在gdb里看了一下，d号chunk正好位于fake_largechunk的末尾(next_fakechunk_addr = 0x58a7511ea770)，内容如下：
```
0x58a7511ea770: 0x0000000000000000      0x0000000000000021
0x58a7511ea780: 0x0000000000000000      0x0000000000000000
0x58a7511ea790: 0x0000000000000000      0x0000000000000021
```
这个布置是必须的，少了任何一个后续都无法正确free掉fake_largechunk

接着是`free fake largechunk`这步。free victim后再alloc victim并修改chunk c的size部分为0x421（fake_largechunk为修改size后的chunk c）。由于前面fakechunk2的布置，chunk c可以正常链入unsorted bin

后面就没啥好说的了。只需要重复以下步骤便能多次利用tcache poisoning：
```py
free(x) #任意chunk即可，主要是为了布置tcache链
free(c) #victim下方的chunk，可以被溢出修改fd
free(victim) #fakechunk
alloc(victim,mangled_address) #想要分配到的地址
alloc(c) #拿回c号chunk
alloc(res,content) #res的地址就是想要分配到的地址
```
佬使用a，b，c标记chunk的做法很直观。我自己做题时总是靠脑子记住哪个chunk在哪个索引，难怪写到后面我自己都头晕 D:

## [House of Water](https://corgi.rip/posts/leakless_heap_1)

效果：
- 劫持`tcache_perthread_struct`的entries，并在`entries`开头放置靠近stdout的libc指针

要求：
1. 有double free或uaf或堆溢出
2. 可精确控制堆布局
3. 可精确控制写入堆块的位置（比如从堆块开头写或者从偏移16字节的地方开始）

该技巧的缺点:
1. 劫持`tcache_perthread_struct`时需要爆破4 bit；后续拿RCE还需要爆破4 bit。成功率1/256
2. 攻击者需要能够读取程序的stdout

原理：
- 如果free第三大和第四大的tcache堆块，`tcache_perthread_struct`的counts字段的末尾与entries字段的开头形成了一个有效的fake chunk header：

![fake_chunk](https://corgi.rip/images/leakless_heap_1/Pasted_image_20241125115127.png)

- 通过一系列堆风水，可以将以上fake chunk链入unsorted bin，从而控制`tcache_perthread_struct`

以下是详细步骤（假设漏洞为UAF）

1. 分配8个0x90大小的堆块，最后一个堆块称为middle
2. 分配大小为0x660（`0x20 + 0x30 + 0x500 + (0x90-8)*2`，但由于glibc的堆块大小计算，似乎实际大小是0x670?）的堆块playground，然后分配大小为0x20的guard1（防止free playground后与top chunk合并）
3. free playground，再分配一个0x20大小的guard2，guard2将从playground中切割。目前堆布局如下：
```
- 7 0x90-sized chunks
- middle (size 0x90)
- guard2 (size 0x20)
...
- top chunk
```
4. 继续切割上述的unsortedbin chunk（原playground），构造如下的堆布局：
```
- 7 0x90-sized chunks
- middle (size 0x90)
- guard2 (size 0x20)
...
- start_M (size 0x90)
- midguard (size 0x30) //防止start_M和end_M合并
- end_M (size 0x90)

- leftovers (size 0x30) //playground剩下的部分
- guard1 (size 0x20)
- top chunk
```
5. 利用uaf修改corruptme的size字段，然后free corruptme。此时（malloc以为的）堆布局如下：
```
- 7 0x90-sized chunks
- middle (size 0x90)
- guard2 (size 0x20)
...
- top chunk
```
6. 可以发现目前的堆布局与第三步相似。重复第四步，但让堆块的布局偏移0x10（corruptme的大小变为0x4e0，leftovers的大小变为0x20，其他保持原样）
7. 伪造fake chunk后面的chunk的prev size和size字段。分配大小为0xf2c8（`0x10080-0xda0-0x18`。此时总共已分配0xda0大小的堆块，而我们希望控制heap_base+0x10080处的内存）的chunk，随后分配0x18大小的chunk得到位于`heap_base+0x10080`的堆块，设置正确的prev size（0x10000）和size（没有具体规定，合理就行，比如0x20）
8. 分配`0x3d8`和`0x3e8`大小的堆块（tcache里第三大和第四大的堆块），然后free，伪造fake chunk的size字段
9. 通过playground处的uaf修改start_M和end_M的size为0x31和0x21，从而在free start_M和end_M后两者进入正确的entry，伪造fake chunk的fd和bk字段。free后需要复原两者的size 0x91。修复时需要直接在start_M和end_M的数据区写，因为后续还要将start（start_M+0x10）和end（end_M+0x10）链入到unsorted bin中。这步有点绕，主要是因为tcache链入chunk时记录的是chunk的用户数据地址，而unsorted bin是metadata地址
10. free第一步分配的7个chunk，然后依次free end，middle和start
11. 修改start的fd和end的bk为fake chunk的地址。即使用2个字节的partial overwrite，此处仍需要爆破4个bit
12. 此时分配`0x888`大小的堆块即可取出fake chunk，控制`tcache_perthread_struct`的entries
13. 后续可以利用partial overwrite得到位于stdout处的堆块从而泄漏地址，然后fsop实现rce